...
#define PKT_DUMP
#define MSDT_UART_BUFFER_SZ     ( 2 * 1024)
...
static void oneshot_timer_callback(void* arg)
{
    ...
    msdt_process_rx_serial_data(serial_rx_data, serial_data_bytes-1);
    serial_data_bytes = 0;
...
static int init_uart()
{
    ...
    uart_driver_install(UART_NUM_2, MSDT_UART_BUFFER_SZ, MSDT_UART_BUFFER_SZ, 10, &uart_queue, 0);
...
void msdt_uart_thread(void *param)
{
    int     s;
    fd_set  rfds;
    struct  timeval tv;
...
{
    xTaskCreate(msdt_uart_thread, "msdt_uart_thread", 4*1024, &context1, 0, &uart_task);
}
